...
```bash
AUDIO_DEVICE_NAME="BlackHole 2ch" npm run test:audio-loopback
```

它会播放一段本地测试音，同时录 BlackHole。只有这个测试通过，才能证明“本机播放声音 -> 多输出设备 -> BlackHole -> ffmpeg”整条链路是通的。输出在：

- `output/audio-loopback/recording.wav`
- `output/audio-loopback/audio_report.json`
...
- `npm run preflight`：检查 BlackHole 设备、Chrome、Playwright、飞书 CLI。
- `npm run test:audio-loopback`：播放本地测试音并确认 BlackHole 能录到声音。

如果你只是临时调页面，可以显式跳过：

```bash
RUN_PREFLIGHT=0 RUN_AUDIO_LOOPBACK=0 REPLAY_URL="你的小鹅通回放链接" npm run run:mvp
```
...
长录过程中默认启用静音 watchdog：先等 30 秒，再每 180 秒抽样录 BlackHole 5 秒。连续 2 次接近静音，会停止长录并在 `audio_watchdog.json` / `record_stop_report.json` 里写明原因。每个任务目录还会写 `job_state.json`，用于判断当前卡在 `preflight`、`page_opened`、`playback_confirmed`、`recording`、`transcribed`、`feishu_done` 的哪一步。
...
如果本机 Playwright 导入卡住，系统会在预检后自动降级到 `open-chrome`。这个兜底模式仍然只打开你已登录可看的页面并录本机音频，但不能自动判断播放器结束，会按 `RECORD_SECONDS` 停止。想禁用自动降级可以设置：
...
启动本机 Runner。推荐用这个命令，不要一直依赖一个普通 Terminal 进程：
